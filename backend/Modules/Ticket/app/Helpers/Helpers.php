...
if (!function_exists('tx_getDate')) {
    ...
    {
        $startCurrentDate = \Carbon\Carbon::now();
        $endCurrentDate = \Carbon\Carbon::now();
        ...
        switch ($sort) {
            ...
            case 'custom':
                if ($startDate && $endDate) {
                    return [
                        'start' => \Carbon\Carbon::createFromFormat('m-d-Y', $startDate)->startOfDay(),
                        'end' => \Carbon\Carbon::createFromFormat('m-d-Y', $endDate)->endOfDay(),
                    ];
                }
                break;
...
if (!function_exists('tx_getPreviousDateRange')) {
    function tx_getPreviousDateRange($sort, $startDate = null, $endDate = null)
    {
        $startCurrentDate = \Carbon\Carbon::now();
        $endCurrentDate = \Carbon\Carbon::now();
        ...
        switch ($sort) {
            case 'today':
                return [
                    'start' => $startCurrentDate->subDay()->startOfDay(),
                    'end' => $endCurrentDate->subDay()->endOfDay(),
                ];

            case 'this_week':
                return [
                    'start' => $startCurrentDate->subWeek()->startOfWeek(),
                    'end' => $endCurrentDate->subWeek()->endOfWeek(),
                ];

            case 'this_month':
                return [
                    'start' => $startCurrentDate->subMonth()->startOfMonth(),
                    'end' => $endCurrentDate->subMonth()->endOfMonth(),
                ];

            case 'this_year':
                return [
                    'start' => $startCurrentDate->subYear()->startOfYear(),
                    'end' => $endCurrentDate->subYear()->endOfYear(),
                ];

            case 'custom':
                if ($startDate && $endDate) {
                    return [
                        'start' => \Carbon\Carbon::createFromFormat('m-d-Y', $startDate)->subYear()->startOfDay(),
                        'end' => \Carbon\Carbon::createFromFormat('m-d-Y', $endDate)->subYear()->endOfDay(),
                    ];
                }
                break;
...
if (!function_exists('calculatePercentage')) {
    function calculatePercentage($current, $previous)
    {
        ...
        return [
            'status'     => $difference > 0 ? 'increase' : ($difference < 0 ? 'decrease' : 'neutral'),
            'percentage' => $rounded . '%',
        ];
    }
}
...
if (!function_exists('tx_getUsersPercentage')) {
    function tx_getUsersPercentage()
    {
        $sort = request()->input('sort');
        $start_date = request()->input('start');
        $end_date = request()->input('end');

        $currentDate = tx_getDate($sort, $start_date, $end_date);
        $previousDate = tx_getPreviousDateRange($sort, $start_date, $end_date);

        $currentCount = User::role(RoleEnum::USER)->whereNull('deleted_at');
        $currentCount = tx_getDateFilter($currentCount, $currentDate, $sort)->count();

        $previousCount = User::role(RoleEnum::USER)->whereNull('deleted_at');
        $previousCount = tx_getDateFilter($previousCount, $previousDate, $sort)->count();

        return calculatePercentage($currentCount, $previousCount);
    }
}

if (!function_exists('tx_getDepartmentsPercentage')) {
    function tx_getDepartmentsPercentage()
    {
        $sort = request()->input('sort');
        $start_date = request()->input('start');
        $end_date = request()->input('end');
        ...
        $currentCount = Department::whereNull('deleted_at');
        $currentCount = tx_getDateFilter($currentCount, $currentDate, $sort)->count();

        $previousCount = Department::whereNull('deleted_at');
        $previousCount = tx_getDateFilter($previousCount, $previousDate, $sort)->count();
...
if (!function_exists('tx_getTicketsPercentage')) {
    function tx_getTicketsPercentage()
    {
        $sort = request()->input('sort');
        $start_date = request()->input('start');
        $end_date = request()->input('end');
        ...
        $currentCount = Ticket::whereNull('deleted_at');
        $currentCount = tx_getDateFilter($currentCount, $currentDate, $sort)->count();

        $previousCount = Ticket::whereNull('deleted_at');
        $previousCount = tx_getDateFilter($previousCount, $previousDate, $sort)->count();
...
if (!function_exists('tx_getOpenTicketsPercentage')) {
    function tx_getOpenTicketsPercentage()
    {
        $sort = request()->input('sort');
        $start_date = request()->input('start');
        $end_date = request()->input('end');
        ...
        $currentCount = Ticket::where('ticket_status_id', $ticketStatusId)->whereNull('deleted_at');
        $currentCount = tx_getDateFilter($currentCount, $currentDate, $sort)->count();

        $previousCount = Ticket::where('ticket_status_id', $ticketStatusId)->whereNull('deleted_at');
        $previousCount = tx_getDateFilter($previousCount, $previousDate, $sort)->count();
