...
use App\Facades\Realtime;
use Exception;
use Illuminate\Support\Facades\Blade;
use Illuminate\Support\ServiceProvider;
use Illuminate\Console\Scheduling\Schedule;
use Modules\Taxido\Console\UpdateDateCommand;
use Modules\Taxido\Console\ProcessNotificationsCommand;
use Modules\Taxido\Console\SendPushNotificationCommand;
use Modules\Taxido\Listeners\SyncDriverStateListener;
...
class TaxidoServiceProvider extends ServiceProvider
{
    ...
    public function broadCastingListener(): void
    {
        Realtime::registerHandler('driverTrack', SyncDriverStateListener::class);
    }

    protected function registerChannels(): void
    {
        $channelsPath = module_path($this->moduleName, 'routes/channels.php');
    ...
    protected function registerCommandSchedules(): void
    {
        $this->app->booted(function () {
            ...
            $schedule->call('Modules\Taxido\Http\Controllers\Api\RideController@offlineDrivers')->everyTwoHours();
            $schedule->call('Modules\Taxido\Http\Controllers\Api\PlanController@verifyIsExpiredSubscriptions')->everyTwoHours();
            $schedule->call('Modules\Taxido\Http\Controllers\Api\DocumentController@updateExpiredDriverDocuments')->everyTwoHours();
            $schedule->call('Modules\Taxido\Http\Controllers\Admin\PushNotificationController@scheduleStatus')->everyTwoHours();
        });
    }
    /**
    ...
    public function registerWidget()
    {
        try {
            ...
            addWidget(
                ...
                function ($data) {
                    return view('taxido::admin.widgets.total-fleet-managers');
                },
                [
                    'context' => 'normal',
                    'priority' => 'high',
    ...
    public function registerQuickLinks()
    {
        try {
            ...
            add_quick_link(('taxido::static.rides.create_ride'), 'admin.ride-request.create', 'ri-steering-2-line', 'ride_request.create');
            add_quick_link(('taxido::static.locations.driver_location'), 'admin.driver-location.index', 'ri-map-pin-line', 'driver_location.index');
            add_quick_link(('taxido::static.drivers.add_driver'), 'admin.driver.create', 'ri-user-line', 'driver.create');
            add_quick_link(('taxido::static.zones.add'), 'admin.zone.create', 'ri-road-map-line', 'zone.create');
