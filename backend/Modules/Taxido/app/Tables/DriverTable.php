...
class DriverTable
{
    ...
    public function getDrivers()
    {
        ...
        if ($this->request->has('driver') && !in_array('all', (array)$this->request->driver)) {
            $drivers = $drivers->whereIn('id', (array)$this->request->driver);
        }

        if ($this->request->has('vehicle_type') && !in_array('all', (array)$this->request->vehicle_type)) {
            $drivers = $drivers->whereHas('vehicle_info', function ($query) {
                $query->whereIn('vehicle_type_id', (array)$this->request->vehicle_type);
            });
        }

        if ($this->request->has('zone') && !in_array('all', (array)$this->request->zone)) {
            $drivers = $drivers->whereHas('zones', function ($query) {
                $query->whereIn('zones.id', (array)$this->request->zone);
            });
        }
        ...
        $drivers = $drivers->where('system_reserve', false)->latest();
        return $drivers;
    }
    ...
    public function getData()
    {
        ...
        if (isset($this->request->s)) {
            $drivers = $drivers->withTrashed()
                ->where(function ($query) {
                    $query->where('name', 'LIKE', "%" . $this->request->s . "%")
                        ->orWhere('email', 'LIKE', "%" . $this->request->s . "%")
                        ->orWhere('phone', 'LIKE', "%" . $this->request->s . "%" );
                });
        }

        if ($this->request->has('orderby') && $this->request->has('order')) {
            $drivers = $drivers->orderBy($this->request->orderby, $this->request->order);
        } else {
            $drivers = $drivers->latest();
        }

        return $this->request->export ? $drivers->get() : $drivers->paginate($this->request?->paginate);
    }
    ...
    public function generate()
    {
        ...
        $drivers->each(function ($driver) {
            if (isDemoModeEnabled()) {
                $driver->email = __('static.demo_mode');
            }

            $driver->role_name = ucfirst($driver->roles->pluck('name')->implode(', '));
            $driver->date = formatDateBySetting($driver->created_at);
        });
        ...
        $tableConfig = [
            ...
            'actionButtons' => [
                ['icon' => 'ri-eye-line', 'route' => 'admin.driver.show', 'class' => 'dark-icon-box', 'permission' => 'driver.index', 'tooltip' => 'Driver details'],
                ['icon' => 'ri-file-2-line', 'route' => 'admin.driver.document', 'class' => 'dark-icon-box', 'permission' => 'driver.index', 'tooltip' => 'Driver document'],
            ],
            'total' => $drivers->count(),
        ];
