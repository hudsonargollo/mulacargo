...
class CouponTable
{
    ...
    public function getData()
    {
        ...
        if ($this->request->has('filter')) {
            switch ($this->request->filter) {
                case 'active':
                    $coupons = $coupons->where('status', true)->orderBy('created_at', 'DESC');
                    break;
                case 'deactive':
                    $coupons = $coupons->where('status', false)->orderBy('created_at', 'DESC');
                    break;
                case 'trash':
                    $coupons = $coupons->withTrashed()->whereNotNull('deleted_at')->orderBy('created_at', 'DESC');
                    break;
            }
        } elseif ($this->request->has('s')) {
            $coupons = $coupons->withTrashed()->where(function ($query) {
                $query->where('title', 'LIKE', "%" . $this->request->s . "%")
                    ->orWhere('code', 'LIKE', "%" . $this->request->s . "%")
                    ->orWhere('type', 'LIKE', "%" . $this->request->s . "%");
            })->orderBy('created_at', 'DESC');
        } elseif ($this->request->has('orderby') && $this->request->has('order')) {
            $coupons = $coupons->orderBy($this->request->orderby, $this->request->order);
        } else {
            $coupons = $coupons->whereNull('deleted_at')->orderBy('created_at', 'DESC');
        }
        ...
        return $coupons->paginate($this->request->paginate);
    }
    ...
    public function generate()
    {
        ...
        $coupons->each(function ($coupon) use($defaultCurrency){
            ...
            $coupon->date = formatDateBySetting($coupon->created_at);
