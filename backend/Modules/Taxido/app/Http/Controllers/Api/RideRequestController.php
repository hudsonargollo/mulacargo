...
class RideRequestController extends Controller
{
    ...
    public function syncCurrentRideRequest(Request $request)
    {
        $this->repository->syncCurrentRideRequest($request);
    }
    ...
    public function filter($rideRequests, $request)
    {
        ...
        if ($request->has('service_type')) {
            $serviceType = is_array($request->service_type) ? $request->service_type : explode(',', $request->service_type);
            $rideRequests = $rideRequests->whereHas('service', function ($query) use ($serviceType) {
                $query->whereIn('type', $serviceType);
            });
        }
