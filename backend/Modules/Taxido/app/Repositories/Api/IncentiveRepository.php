...
class IncentiveRepository extends BaseRepository
{
    ...
    {
        $vehicleTypeZoneId = VehicleTypeZone::where('vehicle_type_id', $request?->vehicle_type_id)
            ->where('zone_id', $request?->zone_id)?->value('id');
    ...
    {
        if ($driver->vehicle_info?->vehicle_type_id) {
            return VehicleTypeZone::where('vehicle_type_id', $driver?->vehicleInfo?->vehicle_type_id)?->value('id');
        }
        return null;
    }
